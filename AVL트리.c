// AVL 트리의 회전 
// 4가지 형식에 의하여 균형이 깨질 수 있음 (균형 인수가 깨지는 노드를 X)

// LL 형식 : X의 왼쪽 자식의 왼쪽에 삽입
// LR 형식 : X의 왼쪽 자식의 오른쪽에 삽입
// RR 형식 : X의 오른쪽 자식의 오른쪽에 삽입
// RL 형식 : X의 오른쪽 자식의 왼쪽에 삽입
...
typedef struct {
	int data;
	int height; // 높이를 저장해야 시간 복잡도 보장 가능
...
Node* rotateLR(Node* node) {
	...
	setHeight(node->leftChild); // 회전 이후에 높이 다시 계산
