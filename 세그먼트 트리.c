...
int init(int start, int end, int node) {
	...
	return tree[node] = init(start, mid, node * 2) + init(mid + 1, node * 2 + 1);
}
...
int sum(int start, int end, int node, int left, int right) {
	...
	int mid = (start + end) / 2;
	return sun(start, mid, node * 2, left, right) + sum(mid + 1, end, node * 2 + 1, left, right);
}
